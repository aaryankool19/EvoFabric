`include "evofabric_defs.svh"

module fir_filter_accel (
    input  logic        clk,
    input  logic        rst_n,
    input  logic        start,
    input  logic        valid,
    input  logic [3:0]  opcode,
    input  logic [31:0] input_a,
    input  logic [31:0] input_b,
    input  logic [31:0] input_c,
    input  logic [31:0] input_d,
    output logic        done,
    output logic        busy,
    output logic [31:0] output_0,
    output logic [31:0] output_1,
    output logic [31:0] output_2,
    output logic [31:0] output_3,
    output logic        error,
    output logic [31:0] cycle_counter,
    output logic [3:0]  accelerator_id
);
    assign accelerator_id = `EVO_ACCEL_FIR;

    logic [1:0] step;
    logic signed [33:0] acc;

    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            done <= 1'b0;
            busy <= 1'b0;
            output_0 <= 32'd0;
            output_1 <= 32'd0;
            output_2 <= 32'd0;
            output_3 <= 32'd0;
            error <= 1'b0;
            cycle_counter <= 32'd0;
            step <= 2'd0;
            acc <= 34'sd0;
        end else begin
            done <= 1'b0;
            if (start && valid && !busy) begin
                busy <= 1'b1;
                cycle_counter <= 32'd1;
                step <= 2'd0;
                acc <= $signed(input_a) + ($signed(input_b) <<< 1);
                error <= (opcode != `EVO_OPCODE_FIR);
            end else if (busy) begin
                cycle_counter <= cycle_counter + 32'd1;
                if (step == 2'd0) begin
                    acc <= acc + ($signed(input_c) * 34'sd3);
                    step <= 2'd1;
                end else begin
                    output_0 <= acc[31:0] + ($signed(input_d) <<< 2);
                    output_1 <= 32'd0;
                    output_2 <= 32'd0;
                    output_3 <= 32'd0;
                    busy <= 1'b0;
                    done <= 1'b1;
                end
            end
        end
    end
endmodule
